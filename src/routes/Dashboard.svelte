...
<script>
  ...
  let stats = $derived([
    { 
      ...
      sub: systemInfo ? systemInfo.cpu_model.split(" ").slice(0, 2).join(" ") : "Loading...", 
      icon: "memory", 
    ...
    { 
      ...
      sub: resourceUsage ? `${resourceUsage.memory_used_gb}GB / ${resourceUsage.memory_total_gb}GB` : "Loading...", 
      icon: "sd_card", 
      color: "text-nx-text-secondary" 
    },
    { 
      ...
      sub: resourceUsage ? `${resourceUsage.disk_used_gb}GB / ${resourceUsage.disk_total_gb}GB` : "Loading...", 
      icon: "hard_drive", 
      color: "text-nx-text-secondary" 
    },
  ]);
...
<div class="mx-auto max-w-5xl">
  ...
  <div class="mb-6 flex items-center justify-between">
    <h1 class="text-xl font-semibold text-nx-text">Overview</h1>
    ...
    {#each stats as stat}
      <div class="border border-nx-border bg-nx-surface p-4">
        <div class="mb-3 flex items-center justify-between">
          <span class="text-sm text-nx-text-secondary">{stat.label}</span>
        ...
        <div class="mt-1 text-xs text-nx-text-muted">{stat.sub}</div>
  ...
  <div class="grid grid-cols-3 gap-4">
    ...
    <div class="col-span-2 border border-nx-border bg-nx-surface">
      <div class="border-b border-nx-border px-4 py-3">
        <h3 class="text-sm font-medium text-nx-text">Recently Used</h3>
      </div>
      <table class="w-full">
        <thead>
          <tr class="border-b border-nx-border text-xs text-nx-text-muted">
            <th class="px-4 py-2 text-left font-medium">Environment</th>
    ...
    <div class="border border-nx-border bg-nx-surface">
      <div class="border-b border-nx-border px-4 py-3">
        <h3 class="text-sm font-medium text-nx-text">System Health</h3>
      </div>
      <div class="space-y-4 p-4">
        ...
        {:else if systemInfo && resourceUsage}
          <div>
            <div class="mb-1 text-xs text-nx-text-muted">Operating System</div>
            <div class="text-sm text-nx-text">{systemInfo.os_name} {systemInfo.os_version}</div>
            <div class="mt-0.5 text-xs text-nx-text-muted">Kernel: {systemInfo.kernel_version}</div>
          </div>
          <div class="h-px bg-nx-border"></div>
          <div>
            <div class="mb-1 text-xs text-nx-text-muted">CPU Usage</div>
          ...
          <div class="h-px bg-nx-border"></div>
          <div>
            <div class="mb-1 text-xs text-nx-text-muted">System Uptime</div>
