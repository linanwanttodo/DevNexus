...
- **Менеджер загрузок** — Многопоточный движок в стиле IDM, докачка, реальная скорость, ускорение через зеркала GitHub, Cookie
...
- **Контейнеры** — Управление Docker/Podman: контейнеры, образы, тома, сети
- **API Hub** — Локальный AI-шлюз, конвертация протоколов OpenAI/Anthropic/Google
...
| [Менеджер загрузок](docs/modules/12-download.md) | Многопоточный движок IDM, очередь работ, реальный прогресс |
...
| [Контейнеры](docs/modules/13-containers.md) | Docker/Podman контейнеры, образы, тома, сети |
...
| [Менеджер версий](docs/modules/10-version.md) | pyenv/fnm/jenv/rustup/gvm |
| [Миграция среды](docs/modules/14-migration.md) | Экспорт/импорт конфигураций |
...
- [x] Менеджер загрузок (многопоточный движок IDM, докачка, реальная скорость)
- [x] Ускорение зеркал GitHub (автоопределение, настраиваемые зеркала, Xget)
- [x] Эмуляция браузера (полные заголовки, Cookie, native-tls)
- [x] Docker/Podman контейнеры
- [x] API Hub (локальный AI-шлюз, мультипротокольная потоковая передача)
- [x] Система миграции окружения
- [x] Бэкенд системного менеджера пакетов (brew / apt / winget)
- [x] UI Центра ПО и интеграция с бэкендом
- [x] Чтение/запись переменных окружения и визуальный редактор
...
- [x] Менеджер версий (pyenv/fnm/jenv/gvm/rustup/gcc)
- [x] Темы и интернационализация (zh / en / ru)
- [x] Автообновление + двуязычный список изменений

### В планах

- [ ] Управление облачными учётными данными (AWS / GCP CLI)
