...
- **Терминал** — Встроенный PTY-терминал с реальными сессиями оболочки
...
- **DevTool Manager** — Только глобальные пакеты npm/cargo/pip, без системного окружения и терминала
- **DevTools-X** — Набор утилит для разработчиков (JSON, JWT и т.д.), не менеджер окружения
- **DevNexus** — **Единственный проект, объединяющий управление пакетами + мульти-runtime + переменные окружения + терминал + настройку зеркал в одном GUI**
...
| **Терминал** | [xterm.js](https://xtermjs.org/) | Веб-рендеринг терминала |
| **Язык бэкенда** | [Rust](https://www.rust-lang.org/) | Системные вызовы, производительность, безопасность памяти |
| **PTY** | [portable-pty](https://crates.io/crates/portable-pty) | Кроссплатформенный бэкенд терминала |
...
- **Rust вместо Node.js** — Нативные системные вызовы, `portable-pty` — самое зрелое кроссплатформенное PTY-решение, безопасность памяти
